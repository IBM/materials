...
from utils import CustomDataset, CustomDatasetMultitask, RMSELoss
...
class Trainer:
    ...
    def __init__(self, raw_data, dataset_name, inputs, target, batch_size, hparams,
                 target_metric='rmse', seed=0, checkpoints_folder='./checkpoints', restart_filename=None, save_every_epoch=False, save_ckpt=True, device='cpu'):
        # data
    ...
    def _prepare_data(self):        
        ...
        self.df_train['string_input'] = self.df_train[self.inputs].apply(lambda row: sep.join(row.values.astype(str)), axis=1)
        self.df_valid['string_input'] = self.df_valid[self.inputs].apply(lambda row: sep.join(row.values.astype(str)), axis=1)
        self.df_test['string_input'] = self.df_test[self.inputs].apply(lambda row: sep.join(row.values.astype(str)), axis=1)

        # self.temp_train = self.df_train['temperature']
        # self.temp_valid = self.df_valid['temperature']
        # self.temp_test = self.df_test['temperature']

        self.df_train = self.df_train.dropna(subset=['string_input'])
        self.df_valid = self.df_valid.dropna(subset=['string_input'])
        self.df_test = self.df_test.dropna(subset=['string_input'])

        # create dataloader
        self.train_loader = DataLoader(
            CustomDataset(self.df_train, self.target),#, self.temp_train), 
            batch_size=self.batch_size, 
            shuffle=True, 
            pin_memory=True
        )
        self.valid_loader = DataLoader(
            CustomDataset(self.df_valid, self.target),#, self.temp_valid), 
            batch_size=self.batch_size, 
            shuffle=False, 
            pin_memory=True
        )
        self.test_loader = DataLoader(
            CustomDataset(self.df_test, self.target),#, self.temp_test), 
            batch_size=self.batch_size, 
            shuffle=False, 
            pin_memory=True
        )
    ...
    def fit(self, max_epochs=500):
        for epoch in range(self.start_epoch, max_epochs+1):
            ...
            if ((val_loss < self.best_vloss) or self.save_every_epoch) and self.save_ckpt:
                ...
                # filename
                self.last_filename = f"STR-Bamba_seed{self.seed}_{self.dataset_name}_epoch={epoch}_valloss={round(val_loss, 4)}.pt"
    ...
    def _load_checkpoint(self, filename):
        ...
        self.model.load_state_dict(ckpt_dict['module'])
    ...
    def _save_checkpoint(self, current_epoch, filename):
        ...
        assert list(ckpt_dict.keys()) == ['module', 'EPOCHS_RUN', 'hparams', 'finetune_info', 'seed']
...
class TrainerRegressor(Trainer):
    ...
    def __init__(self, raw_data, dataset_name, inputs, target, batch_size, hparams,
                 target_metric='rmse', seed=0, checkpoints_folder='./checkpoints', restart_filename=None, save_every_epoch=False, save_ckpt=True, device='cpu'):
        super().__init__(raw_data, dataset_name, inputs, target, batch_size, hparams,
                         target_metric, seed, checkpoints_folder, restart_filename, save_every_epoch, save_ckpt, device) 
    ...
    def _train_one_epoch(self):
        ...
        for idx, data in enumerate(pbar := tqdm(self.train_loader)):
            ...
            with torch.amp.autocast('cuda', dtype=torch.float16, enabled=True):
                ...
                string_input, targets = data
                targets = targets.clone().detach().to(self.device)
                # temp = temp.clone().detach().to(self.device)

                # Make predictions for this batch
                embeddings = self.model.extract_embeddings(string_input).to(self.device)
                # embeddings = torch.cat([embeddings, temp.unsqueeze(-1)], dim=1).to(torch.float16)
    ...
    def _validate_one_epoch(self, data_loader, model=None):
        ...
        with torch.no_grad():
            for idx, data in enumerate(pbar := tqdm(data_loader)):
                ...
                # Make predictions for this batch
                embeddings = model.extract_embeddings(string_input).to(self.device)
                # embeddings = torch.cat([embeddings, temp.unsqueeze(-1)], dim=1).to(torch.float32)
...
class TrainerClassifier(Trainer):
    ...
    def __init__(self, raw_data, dataset_name, inputs, target, batch_size, hparams,
                 target_metric='roc-auc', seed=0, checkpoints_folder='./checkpoints', restart_filename=None, save_every_epoch=False, save_ckpt=True, device='cpu'):
        super().__init__(raw_data, dataset_name, inputs, target, batch_size, hparams,
                         target_metric, seed, checkpoints_folder, restart_filename, save_every_epoch, save_ckpt, device) 
...
class TrainerClassifierMultitask(Trainer):
    ...
    def _train_one_epoch(self):
        ...
        for idx, data in enumerate(pbar := tqdm(self.train_loader)):
            ...
            string_input, targets, target_masks = data
            targets = targets.clone().detach().to(self.device)
