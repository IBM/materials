...
def process_data(idx):
    try:
        dataset = DEFAULTDataset(root_dir='/your/root/directory')
        return dataset[idx]
    except Exception as e:
...
if __name__ == '__main__':
    mp.set_start_method('spawn')  # Ensure 'spawn' start method

    num_processes = 2
    try:
        with mp.Pool(processes=num_processes) as pool:
            results = pool.map(process_data, range(num_processes))
        ...
        # Explicitly close the pool to ensure proper termination
        pool.close()
        pool.join()
