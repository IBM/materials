...
class Smi_ted(nn.Module):
    ...
    def __init__(self, tokenizer, config=None):
        ...
        self.max_len = self.config['max_len']  # TODO: add to FM4M
        self.n_embd = self.config['n_embd']  # TODO: add to FM4M
        self.padding_idx = tokenizer.get_padding_idx()
        self.n_vocab = len(self.tokenizer.vocab)
        self.is_cuda_available = torch.cuda.is_available()
        self.is_xpu_available = torch.xpu.is_available()
        ...
        if self.config:
            ...
            self.net = Net(self.config['n_embd'], n_output=self.config['n_output'] if 'n_output' in self.config else 1, dropout=self.config['d_dropout'])
