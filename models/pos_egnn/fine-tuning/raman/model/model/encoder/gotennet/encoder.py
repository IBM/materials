...
class GATA(MessagePassing):
    def __init__(
        ...
        if layer_norm:
            ...
            print("Scalar layer norm added")
        else:
            self.layernorm = nn.Identity()
        if vector_norm:
            self.tln = TensorLayerNorm(n_atom_basis, trainable=False)
            print("Tensor layer norm added")
        else:
    ...
    def reset_parameters(self):
        if self.layernorm_:
            print("Resetting scalar layer norm parameters")
            self.layernorm.reset_parameters()
        if self.vector_norm_:
            print("Resetting tensor layer norm parameters")
...
# NOTE GotenNet calls hidden_channels n_atom_basis.
# NOTE I kept the same name in the encoder but changed the name in the params to init
# NOTE The config parser _expects_ an n_channels hyperparameter which then adds it as a hyperparameter to the decoder
# NOTE We should change this
class GotenNet(AbstractEncoder):
    def __init__(
        self,
        hidden_channels: int = 128,  # NOTE changed name from original
        num_layers: int = 8,  # NOTE changed name from original
    ...
    def forward(self, data):
        z = data.z
        cutoff_edge_index = data.cutoff_edge_index
        cutoff_edge_distance = data.cutoff_edge_distance
        cutoff_edge_vec = data.cutoff_edge_vec
        ...
        for i, (interaction, mixing) in enumerate(zip(self.gata, self.eqff)):
            ...
            # NOTE Collect all scalars for inter-layer read-outs
        ...
        output_dict = {}
        output_dict["embedding_0"] = layer_outputs.permute(0, 2, 1, 3)  # [n_nodes, n_features, dimension of irrep, n_layers]

        # Store only the last layer embedding
        output_dict["embedding_1"] = mu[:, :3, :].permute(0, 2, 1)  # L1
