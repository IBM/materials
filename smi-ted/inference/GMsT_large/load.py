...
class GMsT(nn.Module):
    """granite.materials.smi-TED-Large 738M Parameters"""
    ...
    def extract_all(self, smiles):
        """Extract all elements from each part of GMsT. Be careful."""
        # evaluation mode
        self.encoder.eval()
        self.decoder.eval()
        if self.is_cuda_available:
...
def load_GMsT(folder="./GMsT_large", 
              ckpt_filename="GMsT-Large_30.pt",
              vocab_filename="bert_vocab_curated.txt"
              ):
    tokenizer = MolTranBertTokenizer(os.path.join(folder, vocab_filename))
    model = GMsT(tokenizer)
